...
export default function HistoryPage() {
  ...
  function exportPdf() {
    ...
    data.slice(0, 25).forEach((item, index) => pdf.text(`${item.strength_input} MPa -> cement 1.000, water ${toRatio(item.predicted_water, item.predicted_cement)}`, 14, 30 + index * 8));
  ...
  return (
    <Card>
      <div className="mb-5 flex flex-col justify-between gap-3 md:flex-row md:items-center">
        <h1 className="text-2xl font-black">Prediction History (Ratios)</h1>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm">
          <thead className="text-slate-500"><tr><th className="py-2">Strength</th><th>Cement</th><th>Slag Ratio</th><th>Fly Ash Ratio</th><th>Water Ratio</th><th></th></tr></thead>
          <tbody>{pageItems.map((item) => <tr key={item.id} className="border-t border-slate-200/70 dark:border-slate-800"><td className="py-3">{item.strength_input}</td><td>{toRatio(item.predicted_cement, item.predicted_cement)}</td><td>{toRatio(item.predicted_blast_furnace_slag, item.predicted_cement)}</td><td>{toRatio(item.predicted_fly_ash, item.predicted_cement)}</td><td>{toRatio(item.predicted_water, item.predicted_cement)}</td><td><button className="btn-secondary size-9 px-0" onClick={() => deleteMutation.mutate(item.id)} aria-label="Delete"><Trash2 size={15} /></button></td></tr>)}</tbody>
        </table>
      </div>
