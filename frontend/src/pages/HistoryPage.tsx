...
export default function HistoryPage() {
  ...
  function exportPdf() {
    ...
    data.slice(0, 25).forEach((item, index) => pdf.text(`${item.strength_input} MPa -> cement ${item.predicted_cement}, water ${item.predicted_water}, age ${item.predicted_age}`, 14, 30 + index * 8));
    ...
    <Card>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm">
          <thead className="text-slate-500"><tr><th className="py-2">Strength</th><th>Cement</th><th>Slag</th><th>Fly Ash</th><th>Water</th><th>Age</th><th></th></tr></thead>
          <tbody>{pageItems.map((item) => <tr key={item.id} className="border-t border-slate-200/70 dark:border-slate-800"><td className="py-3">{item.strength_input}</td><td>{item.predicted_cement}</td><td>{item.predicted_blast_furnace_slag}</td><td>{item.predicted_fly_ash}</td><td>{item.predicted_water}</td><td>{item.predicted_age}</td><td><button className="btn-secondary size-9 px-0" onClick={() => deleteMutation.mutate(item.id)} aria-label="Delete"><Trash2 size={15} /></button></td></tr>)}</tbody>
        </table>
      </div>
