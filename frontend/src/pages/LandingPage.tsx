...
const features = [
  { icon: Sparkles, title: "Inverse mix design", text: "Estimate ingredient quantities from a target compressive strength.", color: "text-amber-500" },
  { icon: Lock, title: "Secure workspace", text: "JWT authentication, bcrypt hashing, protected APIs, and role-ready access.", color: "text-emerald-500" },
  { icon: Database, title: "MongoDB history", text: "Every prediction is saved for review, export, and future analytics.", color: "text-indigo-500" }
];
    ...
    <div className="min-h-screen bg-slate-50/50 text-slate-950 dark:bg-slate-950 dark:text-slate-100">
      ...
      <section className="mx-auto grid max-w-7xl items-center gap-10 px-4 pb-16 pt-10 lg:grid-cols-[1.05fr_0.95fr]">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
          <h1 className="max-w-4xl text-4xl font-black leading-tight tracking-tight sm:text-6xl lg:text-7xl bg-gradient-to-br from-slate-900 via-slate-800 to-cyan-600 bg-clip-text text-transparent dark:from-white dark:to-cyan-400">ConcreteMix AI</h1>
        ...
        <Card className="p-4">
          <div className="rounded-md bg-slate-950 p-5 text-white">
            ...
            {[
              { label: "Cement 340 kg/m³", color: "bg-cyan-400", w: "90%" },
              { label: "Water 185 kg/m³", color: "bg-blue-400", w: "75%" },
              { label: "Coarse Agg. 1120 kg/m³", color: "bg-indigo-400", w: "60%" }
            ].map((item) => (
              <div key={item.label} className="mb-3">
                <div className="mb-1 flex justify-between text-sm"><span>{item.label}</span></div>
      ...
      <section className="mx-auto grid max-w-7xl gap-4 px-4 py-10 md:grid-cols-3">
        {features.map((feature) => (
          <Card key={feature.title} className="hover:border-cyan-500/50 transition-colors">
      ...
      <section className="mx-auto grid max-w-7xl gap-4 px-4 py-10 lg:grid-cols-3">
        <Card><h2 className="font-bold">About Model</h2><p className="mt-2 text-sm text-slate-600 dark:text-slate-300">Multi-output regression trained from concrete compressive strength observations to estimate the mass fraction of seven mix ingredients.</p></Card>
